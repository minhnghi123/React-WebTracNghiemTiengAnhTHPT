...
export const CreateQuestionTypeModal: React.FC<
...
> = ({ visible, handleClose }) => {
  ...
  const createQuestion = async (q: QuestionType) => {
    try {
      console.log("13");
      const rq = await QuestionTypeAPI.createQuestionType(q);
      console.log(rq);
      if (rq?.code === 200) {
        console.log("Thêm loại câu hỏi thành công");
        console.log(rq);
        setQuestion({
          name: "",
          description: "",
        ...
        alert("Thêm dạng câu hỏi thành công");
        handleClose();
      }
    } catch (error: any) {
      if (error.response) {
